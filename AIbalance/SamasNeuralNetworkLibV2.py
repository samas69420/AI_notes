...
# - sigmoide(x,k=1) dove k è il moltiplicatore dell'esponente x
...
# V2
# - forward
# - fusione tra due tensori
# - classe dedidata per la rete neurale
# - round_v per trasformare le liste in un formato tipo [0,0,0,1,0] (forse fa qualcosa di brutto se trova due uguali/ non funziona con 1 elemento)
...
def fondi(W1,W2,w=[1,0,2]): # prende due tensori e li fonde prendendo fino all'elemento prima di w da W1 e da w in poi da W2
    W3=[]
    ...
    #W3[w[0]][w[1]].append(W1[w[0]][w[1]][w[2]]) <-- per includere o escludere l'elemento limite
...
def round_v(l): # da [0.2,0.1,0.5,0.2] ---> [0,0,1,0]
  M = max(l)
...
def single_round(l, soglia):
  for i,element in enumerate(l):
    if element < soglia: l[i]=0
    else: l[i]=1
  return l
...
def check_pesi(pesi, num_check=15, debug=False): # crea 10 nuovi input e controlla che le previsioni della rete siano tutte esatte
  def round_(col):
    lista=to_list(col)
    if lista[0] > lista[1]:
      return [1,0]
...
def setup(totPopu,rete):
    ...
    for i in range(totPopu):
        population.append(SamasNeN(rete))

def make_new_pop(oldpop,mutationrate,debug=False): #dipendenze: crossover, mutate
  l=[]
  if debug: print("oldpop vale: ",oldpop)
  best_ind = oldpop[0]
  second_ind=oldpop[1]
  for i in range(len(oldpop)):
    child=best_ind.crossover(second_ind)
    if debug: print("child vale ", child)
    child.mutate(mutationrate)
    l.append(child)
  if debug: print("l vale: ", l)
  return l

################################################################# NEURAL NETWORK CLASS ###############################################################

class SamasNeN: # init fa una rete neurale senza bias (crea solo il tensore dei pesi partendo dalla lista dei neur.p.l inculdendo l'input)
  ...
  def calcFitness(self, score, distance, debug=False): # uso la sigmoide per essere sicuro di avere un fitness compreso tra 0 e 1
    #self.fitness = a*sigmoide(score,k)
    self.fitness=score-10*abs(distance)
    if debug: print("ho calcolato il fitness e vale ", self.fitness)
  ...
  def mutate(self, mutationRate, debug=False):
    for matrix in self.W:
      for row in matrix:
        for i, element in enumerate(row):
          x = random.uniform(0, 1)
          if (x < mutationRate):
            if debug: print("ho mutato blblblbl")
            row[i] = random.uniform(-1,1)
          """    
          if (x<0.000000001):
            if debug: print("ho super mutato blblblbl")
            row[i] = random.uniform(-1,1)
          """
